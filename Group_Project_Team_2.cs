...
//Jerry-Bot formerly known as GPT_2
//Implements three strategies:
// - Get 1 target at a time at the start of the game
// - At 60 seconds remaining, shift focus to stealing targets from enemy base
// - If it collides with an enemy it will fire its laser and incapacitate the enemy

public class Group_Project_Team_2 : CogsAgent
{
    private bool arrivedEnemyBase = false;
    private int numOfTargetEnemyCaptured = 0;
    ...
    public override void Heuristic(in ActionBuffers actionsOut)
{
    ...
    public override void OnActionReceived(ActionBuffers actions){
        ...
        /**** CODE TO GO TO ENEMY BASE ****/
        int myTeam;
        GameObject enemyBase;
        Transform enemyBaseLocation;
        ...
        if (myTeam == 1) {
            enemyBase = GameObject.Find("Base 2");
        } else {
            enemyBase = GameObject.Find("Base 1");
        }
        enemyBaseLocation = enemyBase.transform;

        // If time is lower than 1 minute, then go to enemy base and steal
        if (timer.GetComponent<Timer>().GetTimeRemaning() < 60) {
            // TODO - If the agent is not carrying anything or at the enemy base, then head to enemy base
            if (!arrivedEnemyBase && GetCarrying() == 0) { // Check if the agent not in enemy base
                if (enemyBaseLocation.position == transform.position) {
                    arrivedEnemyBase = true;
                    // Record how many targets did the enemy captured
                    numOfTargetEnemyCaptured = enemyBase.GetComponent<HomeBase>().GetCaptured();
                }
                TurnAndGo(GetYAngle(enemyBase));
            } else { // If the agent has arrived enemy base
                // Steal
                if(this.GetCarrying() == 0) {
                GoToNearestTarget();
            }
            ...
            // If the time is more than a minute, then pick up the nearest target and head back to base
            if(this.GetCarrying() == 0) {
                GoToNearestTarget();
            }
            if (this.GetCarrying() >= 1) {
...
// ----------------------ONTRIGGER AND ONCOLLISION FUNCTIONS------------------------
    ...
    protected override void OnCollisionEnter(Collision collision) 
    {
        ...
        {
            rewardDict["hit-wall"] += -0.4f;
        }

        if (IsFrozen()) {

            rewardDict["frozen"] += -0.3f;
        }

        // if (!collision.gameObject.CompareTag("Player") && IsLaserOn()) {
        //     rewardDict["shooting-laser"] += -0.5f;
        // }

        if (collision.gameObject.CompareTag("Player")) {
            SetLaser(true);
            rewardDict["hit-enemy"] += 0.2f;
            rewardDict["shooting-laser"] += 0.2f;
        }
    ...
    //  --------------------------HELPERS---------------------------- 
     private void AssignBasicRewards() {
        ...
        rewardDict.Add("hit-wall", 0f);
        rewardDict.Add("got-target", 0f);
    }
